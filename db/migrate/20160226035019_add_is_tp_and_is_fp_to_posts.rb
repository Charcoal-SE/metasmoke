...
class AddIsTpAndIsFpToPosts < ActiveRecord::Migration[4.2]
  def change
    ...
    tp_posts = Feedback.joins(:post).where('feedback_type LIKE \'%tp%\'').group('posts.id').count.map { |k, _v| k }
    fp_posts = Feedback.joins(:post).where('feedback_type LIKE \'%fp%\'').group('posts.id').count.map { |k, _v| k }
