...
class Post < ApplicationRecord
  ...
  after_commit on: :create do
    ...
    post = self
    Thread.new do
      Rails.logger.warn "[autoflagging] #{self.id}: thread begin"
  ...
  def autoflag
    ...
    begin
      ...
      Rails.logger.warn "[autoflagging] #{self.id}: fetched conditions"
      ...
      if users.blank?
        Rails.logger.warn "[autoflagging] #{self.id}: no users available"
        post.send_not_autoflagged
        return 'No users eligible to flag'
      end

      Rails.logger.warn "[autoflagging] #{self.id}: before revision count"
      post.fetch_revision_count
      unless post.revision_count == 1
        Rails.logger.warn "[autoflagging] #{self.id}: vandalized"
        post.send_not_autoflagged
        return 'More than one revision'
      end
      
      Rails.logger.warn "[autoflagging] #{self.id}: lottery begin"
      ...
      core_users_used = []
      Rails.logger.warn "[autoflagging] #{self.id}: core..."
      ...
      Rails.logger.warn "[autoflagging] #{self.id}: plebs..."
      ...
      Rails.logger.warn "[autoflagging] #{self.id}: exception #{e} :("
  ...
  def send_autoflag(user, dry_run, condition)
    Rails.logger.warn "[autoflagging] #{self.id}: send_autoflag begin: #{user.username}" 
    ...
    Rails.logger.warn "[autoflagging] #{self.id}: has enough flags"
    ...
    if last_log.try(:backoff).present? && (last_log.created_at + last_log.backoff.seconds > Time.now)
      Rails.logger.warn "[autoflagging] #{self.id}: flag settings backoff..."
      sleep((last_log.created_at + last_log.backoff.seconds) - Time.now)
    end

    Rails.logger.warn "[autoflagging] #{self.id}: pre spam_flag"
    success, message = user.spam_flag(self, dry_run)
    Rails.logger.warn "[autoflagging] #{self.id}: post spam_flag"
    backoff = 0
    ...
    unless [
      ...
      if success
        Rails.logger.warn "[autoflagging] #{self.id}: send_autoflagged..."
        ...
        Rails.logger.warn "[autoflagging] #{self.id}: broadcast"
      end
    end
  ...
  def send_not_autoflagged
    Rails.logger.warn "[autoflagging] #{self.id}: send_not_autoflagged..."
  ...
  def fetch_revision_count(post = nil)
    Rails.logger.warn "[autoflagging] #{self.id}: fetch_revision_count begin"
    post ||= self
    return unless post.site.present?
    Rails.logger.warn "[autoflagging] #{self.id}: site was present"
    ...
    Rails.logger.warn "[autoflagging] #{self.id}: queried SE: #{revision_list&.count}"
