...
class User < ApplicationRecord
  ...
  def get_username(readonly_api_token = nil)
    ...
    begin
      ...
      resp = StackAPIHelper.get_response(URI.parse("https://api.stackexchange.com/2.2/me/associated?pagesize=1&filter=!ms3d6aRI6N&#{auth_string}"))
      resp = JSON.parse(resp.body)

      first_site = URI.parse(resp['items'][0]['site_url']).host

      resp = StackAPIHelper.get_response(URI.parse("https://api.stackexchange.com/2.2/me?site=#{first_site}&filter=!-.wwQ56Mfo3J&#{auth_string}"))
      resp = JSON.parse(resp.body)
  ...
  def update_moderator_sites
    ...
    while has_more
      ...
      response = JSON.parse(StackAPIHelper.get_response(URI.parse(url)).body)
