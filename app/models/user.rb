...
class User < ApplicationRecord
  ...
  def flag(flag_type, post, dry_run = false)
    ...
    flag_option = flag_options.select do |fo|
      if flag_type.to_sym == :spam
        ...
        ['very low quality', 'qualidade muito baixa', 'muy baja calidad', '品質が低すぎる', 'необходимо удалить из-за проблем с качеством'].include? fo['title']
      else
  ...
  def vlq_flag(post, dry_run = false)
    flag :vlq, post, dry_run
  end
