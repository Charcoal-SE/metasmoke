class HairOnFireMailer < ApplicationMailer
  ...
  def smokey_down_email(smoke_detector)
    ...
    mail(to: smoke_detector.user.try(:email) || 'undo@erwaysoftware.com', cc: 'teward@ubuntu.com; undo@erwaysoftware.com; hello@artofcode.co.uk', subject: "[SmokeDetector] status DOWN (last seen #{@last_ping_date})")
  end
end
