class FlagLogController < ApplicationController
  ...
  def index
    ...
    @flag_logs = @applicable_flag_logs.order('flag_logs.created_at DESC, flag_logs.id DESC').includes(:post => [:feedbacks => [:user, :api_key]]).includes(:post => [:reasons]).includes(:user).page(params[:page])
    @sites = Site.where(:id => @flag_logs.map(&:post).map(&:site_id)).to_a
  end

  def by_post
    @individual_post = Post.find(params[:id])
    @flag_logs = @individual_post.flag_logs.order('created_at DESC, id DESC').includes(:post => [:feedbacks => [:user, :api_key]]).includes(:post => [:reasons]).includes(:user).page(params[:page])
    @sites = Site.where(:id => @flag_logs.map(&:post).map(&:site_id)).to_a
    render :index
  end
  ...
  def not_flagged
    @posts = Post.left_joins(:flag_logs).where(:flag_logs => { :id => nil }).includes(:feedbacks => [:user, :api_key]).includes(:reasons).order(:created_at => :desc).page(params[:page])
