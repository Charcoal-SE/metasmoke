...
class GithubController < ApplicationController
  ...
  def pull_request_hook
    ...
    domains.each do |domain|
      num_tps = Post.where('body LIKE ?', sanitize_sql_like('%#{domain}%')).where(is_tp: true).count
      num_fps = Post.where('body LIKE ?', sanitize_sql_like('%#{domain}%')).where(is_fp: true).count
      num_naa = Post.where('body LIKE ?', sanitize_sql_like('%#{domain}%')).where(is_naa: true).count
    ...
    keywords.each do |keyword|
      num_tps = Post.where('body LIKE ?', sanitize_sql_like('%#{keyword}%')).where(is_tp: true).count
      num_fps = Post.where('body LIKE ?', sanitize_sql_like('%#{keyword}%')).where(is_fp: true).count
      num_naa = Post.where('body LIKE ?', sanitize_sql_like('%#{keyword}%')).where(is_naa: true).count
    ...
    usernames.each do |username|
      num_tps = Post.where('body LIKE ?', sanitize_sql_like('%#{username}%')).where(is_tp: true).count
      num_fps = Post.where('body LIKE ?', sanitize_sql_like('%#{username}%')).where(is_fp: true).count
      num_naa = Post.where('body LIKE ?', sanitize_sql_like('%#{username}%')).where(is_naa: true).count
