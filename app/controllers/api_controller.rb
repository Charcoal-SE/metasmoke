class ApiController < ApplicationController
  ...
  def posts
    ...
    @posts = Post.where(:id => params[:ids].split(";")).select(select_fields(filter)).order(:id => :desc).left_joins(:feedbacks).left_joins(:deletion_logs).includes(:flag_logs => [:user]).group(:id)
